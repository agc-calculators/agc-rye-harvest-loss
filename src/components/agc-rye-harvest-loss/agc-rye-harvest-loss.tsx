...
@Component({
    tag: 'agc-rye-harvest-loss'
})
export class AgcRyeHarvestLoss {
    ...
    render() {
        return (
            <div>
                <form onSubmit={(e) => e.preventDefault()} ref={c => this.form = c as HTMLFormElement} data-wizard="agc-rye-harvest-loss" 
                    data-wizard-mode={this.mode}
                    class="agc-wizard">
                    <slot></slot>
                    <section data-wizard-section="0">
                      <div class="agc-wizard__field">
                        <label data-i18n="fields.kernel-count">Kernel Count</label>
                        <input name="kernelCount" type="number" required min="0" step="1" />
                        <p class="agc-wizard__validation-message" data-i18n="validation.kernel-count.required" data-validates="kernelCount">Please enter a whole number of zero or greater.</p>
                        <p data-i18n={`hints.kernel-count`}>⮤ Enter the number of rye kernels counted on the ground within the specified area.</p>
                      </div>
                    ...
                    <section data-wizard-section="1">
                      <div class="agc-wizard__field">
                        ...
                        <p class="agc-wizard__validation-message" data-i18n="validation.loss-area.required" data-validates="lossArea">Please enter a whole number of zero or greater.</p>
                        <p data-i18n={`hints.loss-area`}>⮤ Enter the area in square feet that was used to count rye kernels on the ground.</p>
                      </div>
                      <div class="agc-wizard__actions">
	...
	if (!validate(this.form, 'kernelCount')) {
                valid = false
            }
        }     
    ...
    showResults() {
            ...
            let kernelCount=  parseInt((this.form.querySelector('[name="kernelCount"') as HTMLInputElement).value);                    
            let lossArea=  parseInt((this.form.querySelector('[name="lossArea"') as HTMLInputElement).value);                    
              let yieldLoss = round(kernelCount / lossArea / 22, 2)      
